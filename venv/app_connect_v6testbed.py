...
def reschedule_task_to_empty_slot():
    ...
    try:
        ...
        task_rescheduling_check_delay = 15
...
def create_events_from_items( items, event_date):
    ...
    print(f"SKIPPED for testing : create_events_from_items method")
    return
...
def delete_all_events(service, event_date, time_min, time_max):
    ...
    print(f"SKIPPED for testing : delete_all_events method")
    return
    service = gcal_auth()
...
def extract_existing_event_data(existing_events, pst_offset):
    print(f"SKIPPED for testing : extract_existing_event_data method")
    return
...
def compare_and_sync_events( items, event_date):
    print(f"SKIPPED for testing : compare_and_sync_events method")
    return
...
def run_pygsheets():
    ...
    worksheet_name = 'testtimetable'  # orignal sheet
    current_cell = {
        'row_index': None,
        'col_index': None
    }
